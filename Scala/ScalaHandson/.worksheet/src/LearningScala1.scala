object LearningScala1 {;import org.scalaide.worksheet.runtime.library.WorksheetSupport._; def main(args: Array[String])=$execute{;$skip(126); 
  ...
  val isBest: Boolean = picard == bestCaptain;System.out.println("""isBest  : Boolean = """ + $show(isBest ));$skip(334); 
  ...
  def doublePi(pi:Float) : Float  = {
     pi*2
  };System.out.println("""doublePi: (pi: Float)Float""");$skip(62); 
  
  println(f"pi double value ${doublePi(3.14159265f)}%.3f")}
}
